...
export function AnalyticsClient({ tasks }: AnalyticsClientProps) {
   ...
   return (
      <div className="space-y-8 pb-20">
         <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
               <h2 className="text-3xl font-display font-bold">Task Analytics</h2>
               <p className="text-muted-foreground text-sm font-medium">Deep inspection of departmental throughput and bottleneck detection.</p>
            </div>
            <div className="flex gap-2">
               <Badge variant="outline" className="bg-emerald-500/10 border-emerald-500/20 text-emerald-400 font-bold uppercase tracking-widest text-[10px]">
                  System Health: Optimal
               </Badge>
               <Badge variant="outline" className="bg-white/5 border-white/10 text-white font-bold uppercase tracking-widest text-[10px]">
                  Period: Custom (24h)
               </Badge>
            </div>
         </div>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <Card className="glass-card border-white/5 bg-white/5 overflow-hidden">
               <CardHeader>
                  ...
                  <CardDescription className="text-[10px] uppercase font-black">Daily task completion vs pending load</CardDescription>
               </CardHeader>
            ...
            <Card className="glass-card border-white/5 bg-white/5">
               <CardHeader>
                  ...
                  <CardDescription className="text-[10px] uppercase font-black">Unresolved high-priority missions</CardDescription>
               </CardHeader>
               <CardContent className="flex flex-col md:flex-row items-center justify-around h-[400px]">
                  ...
                  <div className="w-full md:w-1/2 space-y-4 px-8">
                     <div className="p-4 rounded-2xl bg-white/5 border border-white/5">
                        <p className="text-[10px] font-black uppercase text-amber-400 mb-1">Critical Insight</p>
                        ...
                        {priorityPieData.map((d, i) => (
                           <div key={i} className="flex justify-between items-center text-xs">
                              <span className="flex items-center gap-2">
                                 ...
                                 <span className="font-bold opacity-70">{d.name}</span>
                              </span>
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="glass-card border-white/5 bg-white/5 p-6 space-y-2">
               <div className="flex items-center gap-3">
                  <Clock className="w-5 h-5 text-indigo-400" />
                  <h3 className="text-sm font-bold uppercase tracking-widest text-indigo-400">Mean Lead Time</h3>
               </div>
               <p className="text-3xl font-black">1.2 <span className="text-xs text-muted-foreground uppercase">Days</span></p>
               <p className="text-[10px] text-emerald-400 font-bold uppercase">↓ 14% from last week</p>
            </Card>
            ...
            <Card className="glass-card border-white/5 bg-white/5 p-6 space-y-2">
               ...
               <p className="text-3xl font-black">8.4 <span className="text-xs text-muted-foreground uppercase">Tasks/Day</span></p>
               <p className="text-[10px] text-emerald-400 font-bold uppercase">↑ 8% from last week</p>
            </Card>
