...
export function SupervisoryClient({ profile, initialTasks, initialAttendance, team }: SupervisoryClientProps) {
  ...
  return (
    <AdminViewWrapper
      title={roleMetadata.label + " Command"}
      subtitle="Field operations oversight and tactical team coordination."
      badgeLabel="OPERATIONS LEAD"
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card className="glass-card border-white/5 bg-white/5 overflow-hidden group">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Active Ops</p>
                <Activity className="w-4 h-4 text-emerald-400" />
              </div>
              <p className="text-3xl font-display font-bold">{activeTasks}</p>
              <p className="text-[10px] text-emerald-400 mt-1 font-bold">LIVE MISSIONS</p>
            </CardContent>
          </Card>
          ...
          <Card className="glass-card border-white/5 bg-white/5 overflow-hidden group">
            <CardContent className="p-6">
              ...
              <p className="text-[10px] text-indigo-400 mt-1 font-bold">DIRECT PERSONNEL</p>
            </CardContent>
          </Card>
          ...
          <Card className="glass-card border-white/5 bg-white/5 overflow-hidden group">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Blockers</p>
                <Zap className="w-4 h-4 text-amber-400" />
              </div>
              <p className="text-3xl font-display font-bold">{blockedTasks}</p>
              <p className="text-[10px] text-amber-400 mt-1 font-bold">ATTENTION REQUIRED</p>
            </CardContent>
          </Card>
          ...
          <Card className="glass-card border-white/5 bg-white/5 overflow-hidden group">
            <CardContent className="p-6">
              ...
              <p className="text-[10px] text-sky-400 mt-1 font-bold">SHIFT THROUGHPUT</p>
            </CardContent>
          </Card>
        </div>
               ...
               <div className="space-y-4">
                  {tasks.slice(0, 5).map((t, i) => (
                    <div key={i} className="flex gap-3 text-[11px] p-3 rounded-xl bg-white/[0.02] border border-white/5">
                      <div className={`w-1 h-full rounded-full ${t.priority === 'high' ? 'bg-red-500' : 'bg-indigo-500'}`} />
                      <div className="space-y-1">
                        <p className="font-bold text-indigo-100/80">{t.title}</p>
                        <p className="text-muted-foreground opacity-60">Status updated to {t.status.replace('_', ' ')}</p>
                      </div>
                    </div>
                  ))}
               </div>
