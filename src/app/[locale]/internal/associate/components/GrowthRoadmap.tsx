    ...
    <div className="space-y-10">
      ...
      <div className="relative space-y-4">
        {checklist.length > 0 ? checklist.map((item, i) => (
          ...
          >
            <Card
              className={`glass-card border-white/5 transition-all duration-300 cursor-pointer ${item.is_completed ? 'bg-emerald-500/5 border-emerald-500/20 opacity-60' : 'hover:border-indigo-500/30'
                }`}
              onClick={() => handleToggleChecklist(item.id, item.is_completed)}
            >
              <CardContent className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors ${item.is_completed ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-white/20'
                    }`}>
