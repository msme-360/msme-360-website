...
export default function OperationalHero({
  ...
  const t = useTranslations("Associate.hero");
  ...
  const handleAttendance = async (type: 'in' | 'out') => {
    ...
    if (res.success && res.data) {
      toast.success(type === 'in' ? t("checkedIn") : t("checkedOut"));
      ...
      toast.error(res.error || t("failure"));
    }
    setLoading(false);
  };
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center gap-12">
        ...
        <div className="space-y-4 text-center md:text-left flex-1">
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 mb-2">
            ...
            {promotion && (
              <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30 px-4 py-1.5 text-xs font-bold tracking-widest uppercase rounded-full animate-pulse">
          ...
          <h1 className="text-4xl md:text-5xl font-display font-black tracking-tight text-white leading-tight">
            {t.rich("welcome", {
              ...
              span: (chunks) => <span className="text-indigo-400">{chunks}</span>
            })}
          </h1>
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-6 mt-6">
            <div className="flex items-center gap-3">
              {hasFinishedDay ? (
                ...
                >
                  {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : t("initiateCheckout")}
                </Button>
              ) : (
                <Button
                ...
                >
                  {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : t("deployToSite")}
                </Button>
              )}
            </div>
            ...
            <div className="text-left space-y-1">
              <p className="text-[10px] font-black uppercase tracking-widest text-indigo-400/60">{t("shiftOverview")}</p>
