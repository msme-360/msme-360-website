...
export default function TacticalHub({
  ...
  const t = useTranslations("Associate.hub");
  return (
    <Card className="glass-card border-white/10 overflow-hidden shadow-2xl relative">
      {!hideTitle && (
        <>
          <div className="absolute top-0 right-0 p-3">
            <Badge variant="outline" className="bg-indigo-500/10 border-indigo-500/20 text-indigo-400 text-[8px]">{t("activeOps")}</Badge>
          </div>
      ...
      <CardContent className="p-0">
                  ...
                  <SheetContent className="glass-card border-white/10 text-white w-[400px] sm:w-[540px]">
                    <SheetHeader>
                      ...
                      <div className="flex gap-2 pb-4">
                        <Badge variant="outline" className="text-[10px] uppercase font-black tracking-widest">{t(`status.${task.status}`)}</Badge>
                        <Badge variant="outline" className="text-[10px] uppercase font-black tracking-widest text-indigo-400">{t(`priority.${task.priority}`)}</Badge>
                      </div>
                    </SheetHeader>

                    <div className="space-y-6 pt-6 h-full flex flex-col">
                      <div className="space-y-2">
                        <Label className="text-[10px] font-black uppercase text-indigo-400">{t("briefing")}</Label>
                        <p className="text-sm text-indigo-100/60 leading-relaxed bg-white/5 p-4 rounded-xl border border-white/5">
                          {task.description || t("noBriefing")}
                        </p>
                      </div>

                      <div className="flex-1 flex flex-col min-h-0">
                        <Label className="text-[10px] font-black uppercase text-indigo-400 mb-4">{t("commsRelay")}</Label>
                      ...
                      <div className="pt-6 space-y-3">
                        <Label className="text-[10px] font-black uppercase text-emerald-400">{t("pow")}</Label>
                        <div className="flex gap-2">
                          <input
                            placeholder={t("placeholders.pow")}
                            defaultValue={task.proof_of_work || ""}
                            className="flex-1 bg-white/5 border-white/10 rounded-xl px-4 text-xs focus:ring-emerald-500 text-white"
                            id={`pow-${task.id}`}
                          />
                          <Button
                            size="sm"
                            onClick={() => {
                              const input = document.getElementById(`pow-${task.id}`) as HTMLInputElement;
                              onUpdatePoW(task.id, input.value);
                            }}
                            className="bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 border border-emerald-500/30 h-10"
                          >
                            {t("submit")}
                          </Button>
                        </div>

                        <Label className="text-[10px] font-black uppercase text-indigo-400">{t("commsRelay")}</Label>
                        <Textarea
                        ...
                        >
                          {isSubmittingComment ? <Loader2 className="w-4 h-4 animate-spin" /> : t("transmitRelay")}
                ...
                <div className="flex justify-between items-start mb-2 relative z-10 pointer-events-none">
                  <div className="flex items-center gap-2">
                    <span className="text-[9px] font-black uppercase tracking-widest text-indigo-400">
                      {t("priority.label", { value: t(`priority.${task.priority}`) })}
                    </span>
                    {task.due_date && (
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-muted-foreground truncate opacity-60">
                      {t("assignedBy", { name: task.assigned_by_profile?.full_name || 'Admin' })}
                    </p>
                  </div>
                </div>
              </motion.div>
        ...
        <div className="p-4 bg-indigo-500/5 border-t border-indigo-500/10">
          <Button
            variant="ghost"
            onClick={() => toast.info(t("missionRequested"), {
              description: t("missionRequestedDesc")
            })}
