...
import { getProfile as getProfileDirect, getProgress } from "@/app/[locale]/dashboard/queries";
...
async function IdentityPortalContent({
  ...
  const onboardingPercentage = Math.round((progress.filter(s => s.completed).length / progress.length) * 100);
  ...
  if (!requestedRole) {
    redirect(`/${locale}/internal/identity/${userRole}`);
  }
