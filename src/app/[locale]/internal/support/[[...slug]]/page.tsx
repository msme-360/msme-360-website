...
async function SupportPortalContent({
  ...
  // 1. Role Silo Check
  if (!requestedRole) {
    redirect(`/${locale}/internal/support/${userRole}`);
  }
  ...
  const [initialTickets, supportMetrics] = await Promise.all([
    getSupportTickets(userRole, profile?.department),
    getPlatformMetrics('support')
  ]);
