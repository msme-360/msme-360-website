          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            {competitiveChecklistItems.map((item) => (
              <div
                ...
                className={`flex items-center gap-3 p-4 rounded-xl border transition-all cursor-pointer group/item ${completedItems.includes(item.id)
                  ? 'bg-primary/10 border-primary/30'
                  : 'bg-white/5 border-white/5 hover:border-primary/20 hover:bg-white/10'
                  }`}
              >
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${completedItems.includes(item.id)
