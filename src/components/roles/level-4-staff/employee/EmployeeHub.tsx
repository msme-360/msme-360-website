...
export default function EmployeeHub({ metrics = [] }: { metrics?: DashboardMetric[] }) {
  return (
      ...
      <CardHeader className="bg-slate-500/5 border-b border-slate-500/10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-xl font-bold tracking-tight text-white">Execution Hub</CardTitle>
              <p className="text-xs text-muted-foreground uppercase font-black tracking-widest mt-0.5">Operations Intelligence</p>
            </div>
          </div>
          <Badge className="bg-slate-500/20 text-slate-400 border-slate-500/30 font-bold tracking-widest">L4 Staff</Badge>
        </div>
      </CardHeader>
