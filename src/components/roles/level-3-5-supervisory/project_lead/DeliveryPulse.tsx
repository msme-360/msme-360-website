...
export default function DeliveryPulse({ metrics = [] }: { metrics?: DashboardMetric[] }) {
  return (
    <Card className="glass-card border-blue-500/20 bg-blue-500/[0.02] overflow-hidden">
      <CardHeader className="bg-blue-500/5 border-b border-blue-500/10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-xl font-bold tracking-tight text-white">Delivery Pulse</CardTitle>
              <p className="text-xs text-muted-foreground uppercase font-black tracking-widest mt-0.5">Management Intelligence</p>
            </div>
          </div>
          <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/30 font-bold tracking-widest">L3.5 Supervisor</Badge>
        </div>
      </CardHeader>
        ...
        {metrics && metrics.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {metrics.slice(0, 4).map((m: DashboardMetric, idx: number) => (
              <div key={m.label || idx} className="p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:border-blue-500/20 transition-all group">
