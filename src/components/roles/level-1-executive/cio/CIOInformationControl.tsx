...
export default function CIOInformationControl({ metrics = [] }: { metrics?: DashboardMetric[] }) {
  return (
    <Card className="glass-card border-cyan-500/20 bg-cyan-500/[0.02] overflow-hidden">
      <CardHeader className="bg-cyan-500/5 border-b border-cyan-500/10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-xl font-bold tracking-tight text-white">Information Control</CardTitle>
              <p className="text-xs text-muted-foreground uppercase font-black tracking-widest mt-0.5">Technical Intelligence</p>
            </div>
          </div>
          <Badge className="bg-cyan-500/20 text-cyan-400 border-cyan-500/30 font-bold tracking-widest">L1 EXEC</Badge>
        </div>
      </CardHeader>
        ...
        {metrics && metrics.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {metrics.slice(0, 4).map((m: DashboardMetric, idx: number) => (
              <div key={m.label || idx} className="p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:border-cyan-500/20 transition-all group">
