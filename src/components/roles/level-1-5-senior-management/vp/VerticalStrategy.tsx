...
export default function VerticalStrategy({ metrics = [] }: { metrics?: DashboardMetric[] }) {
  return (
      ...
      <CardHeader className="bg-indigo-500/5 border-b border-indigo-500/10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-xl font-bold tracking-tight text-white">Vertical Strategy</CardTitle>
              <p className="text-xs text-muted-foreground uppercase font-black tracking-widest mt-0.5">Management Intelligence</p>
            </div>
          </div>
          <Badge className="bg-indigo-500/20 text-indigo-400 border-indigo-500/30 font-bold tracking-widest">L1.5 EXEC</Badge>
        </div>
      </CardHeader>
